...
function GetRights() {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const updatedAdminList = await adminApiService.updateRights(accessToken.token, updateData);
  ...
  const handleFieldChange = (fieldName, value) => {
    ...
    setUpdateField(prev => ({ ...prev, role_type_master_id: parseInt(value) }));
    ...
    <>
      <div className="center-container">
        ...
        <p className="table-heading">
          Define rights to role
        </p>


        <div className="table-container" style={{width:"400px"}}>
      ...
      >
        <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 500, bgcolor: 'background.paper', boxShadow: 24, p: 4 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <h2 id="modal-modal-title">Edit Details</h2>
            <img src={close} className="Examtype-close-btn" alt="Close" onClick={closeModal} style={{ cursor: 'pointer' }} />
          </div>
          <form onSubmit={handleUpdate}>
            ...
            <select
              ...
              onChange={(e) => handleFieldChange('role_type_master_id', e.target.value)}
            >
              <option value="">Select Role Type</option>
              {role.map((role) => (
                <option key={role.id} value={role.id}>
